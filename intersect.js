...
const settings = {
  defaultConcernTimeFrameMinutes: ,
  defaultThresholdMatchPercent: ,
  maxExposureWindowDays: ,
  minCheckIntersectInterval: 
}

// Errol to figure out data structure and replace with JS objects
import {
...
export function discardOldData(
  localDataPoints = [],
  exposureWindowDays = maxExposureWindowDays,
) {
...
const getTransformedLocalDataAndDayBins = async (gpsPeriodMS) => {
  ...
  const locationArray = "x"; // --- insert locations
...
export function fillDayBins(
  dayBins,
  localGPSDataPoints,
  concernTimeFrameMS = defaultConcernTimeFrameMinutes * 60e3,
  thresholdMatchPercent = defaultThresholdMatchPercent,
  gpsPeriodMS = MIN_LOCATION_UPDATE_MS,
) {
...
async function asyncCheckIntersect(healthcareAuthorities, bypassTimer = false) {
  ...
  if (
    !bypassTimer &&
    lastCheckedMs + minCheckIntersectInterval * 60e3 > dayjs().valueOf()
  ) {
    return null;
  }
...
export function enableDebugMode() {
  ...
  let pseudoBin = [];
  for (let i = 0; i < maxExposureWindowDays; i++) {
    let intersections =
